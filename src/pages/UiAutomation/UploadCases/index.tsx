...
export default function UploadCases({ setTestCase }: { setTestCase: (val: string) => void }) {
  const [template, setTemplate] = useState<string>("");
    ...
    <div className="bg-[#1A1B2E] rounded-lg p-8 w-full max-w-5xl border border-white/20 shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {templates.map((templateItem, index) => (
          <div
            key={index}
            className="bg-[#0D0D1A] rounded-lg p-6 border border-white/20 hover:border-blue-600 
                       flex flex-col items-center text-center cursor-pointer transition-all shadow-md hover:shadow-xl"
            onClick={() => {
              setTemplate(templateItem?.title);
              
            }}
          >
            {templateItem.icon}
            <h3 className="text-white font-semibold text-lg mt-4">{templateItem.title}</h3>
            <p className="text-gray-300 text-sm mt-2">{templateItem.description}</p>
            <p className="text-gray-500 text-xs mt-1">{templateItem.subtext}</p>
          </div>
        ))}
      </div>
